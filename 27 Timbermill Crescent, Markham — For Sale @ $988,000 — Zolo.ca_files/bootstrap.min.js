 ...
 * plugins: bootstrap-transition.js, bootstrap-modal.js, bootstrap-dropdown.js, bootstrap-tab.js, bootstrap-tooltip.js, bootstrap-button.js, bootstrap-carousel.js, bootstrap-typeahead.js
...
! function(a) {
	...
	b.prototype = {
		...
		select: function() {
			...
			var searchValue = '';
			if ( MOBILEDETECT.isZoloMobile() ) {
				searchValue = document.getElementById( 'mobile_sarea' ).value;
				if ( searchValue !== '' && searchValue.length > 0 ) {
					if ( document.getElementById( 'sarea' ) instanceof HTMLElement ) { 
						document.getElementById( 'sarea' ).value = searchValue;
						document.getElementById( 'sarea_hidden' ).value = searchValue;
					}
					$("form[name=home_search_top]").submit();
				}
			} else {
				if ( document.getElementById( 'sarea' ) instanceof HTMLElement ) { 
					searchValue = document.getElementById( 'sarea' ).value;
				}
				if ( searchValue !== '' && searchValue.length > 0 ) {
					document.getElementById( 'sarea_hidden' ).value = searchValue;
					$("form[name=home_search_top]").submit();
		...
		show: function() {
			$(".mobile-nearby-cities-search, .mobile-popular-search-search").attr("style", "display: none !important");
			if ( MOBILEDETECT.isZoloMobile() ) {
				this.$menu.insertBefore($("#mobile-other-searches-menu"));
				this.$menu.show();
				this.shown = !0;
				return this;
			} else {
		...
		hide: function() {
			$(".mobile-nearby-cities-search, .mobile-popular-search-search").attr("style", "");
			this.$menu.hide();
			this.shown = !1;
			return this;
		},
		lookup: function(b) {
			...
			if (this.$element.val().length > 0) {
				$("#clear-sarea-input-field").attr("style", "margin-right:-1rem;");
			} else {
				$("#clear-sarea-input-field").attr("style", "display: none !important");
			}
		...
		process: function(b) {
			...
			b = this.sorter(b);
			var me = this;
			// var hideme = setInterval(function() {
			// 	me.hide();
		...
		render: function(b) {
			...
			b = a(b).map(function(b, d) {
				return b = a(c.options.item).attr("data-value", d), b.find(".truncate").html(c.highlighter(d)), b[0]
			});
	...
	a.fn.typeahead = function(c) {
		...
		source: [],
		items: 8,
		menu: '<ul class="typeahead dropdown-menu xs-full-width list-unstyled xs-border-none md-border"></ul>',
		item:   '<li><a class="button button--lookahead xs-full-width xs-text-left xs-flex xs-flex-align-center" href="#">' +
					'<i class="icon icon-location-on text-muted xs-mr3" style="font-size:24px;height:24px;width:24px;"></i>' +
					'<div class="truncate">' +
						// '<span><strong>Burn</strong>aby BC</span>' +
					'</div>' +
				'</a></li>',
		minLength: 1
